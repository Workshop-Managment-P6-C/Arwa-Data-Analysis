...
CREATE TABLE `hall` (
  `id` CHAR(36) PRIMARY KEY DEFAULT (UUID()),
  `name` varchar(255) UNIQUE NOT NULL,
  `kind` varchar(255) COMMENT 'lecture or lab space used by academy training',
  `is_active` boolean DEFAULT true
);
...
ALTER TABLE `bay` COMMENT = 'A physical workshop bay (4 in the demo). Purpose: the workshop resource that jobs book; academy training sessions use halls instead.';
...
ALTER TABLE `hall` COMMENT = 'A training hall (4 in the demo). Purpose: the room academy sessions are scheduled in, separate from workshop bays.';

ALTER TABLE `hall_booking` COMMENT = 'Hall calendar for training sessions. Purpose: detect hall conflicts. MySQL: check overlaps in the service.';
...
ALTER TABLE `training_session` COMMENT = 'One scheduled class: course, group, session title, hall, mentor and time. Purpose: the unit that is checked for bay and mentor conflicts before it can be published.';
